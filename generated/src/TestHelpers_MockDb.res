...
@genType
and entities = {
    @as("Pair") pair: entityStoreOperations<Entities.Pair.t>,
    @as("PancakeFactory") pancakeFactory: entityStoreOperations<Entities.PancakeFactory.t>,
    @as("PancakeFactory_PairCreated") pancakeFactory_PairCreated: entityStoreOperations<Entities.PancakeFactory_PairCreated.t>,
    @as("PancakePair_Swap") pancakePair_Swap: entityStoreOperations<Entities.PancakePair_Swap.t>,
    @as("PancakePair_Sync") pancakePair_Sync: entityStoreOperations<Entities.PancakePair_Sync.t>,
  ...
  let entities = {
      ...
      pancakeFactory_PairCreated: {
        makeStoreOperatorEntity(
          ~inMemoryStore,
          ~makeMockDb=makeWithInMemoryStore,
          ~getStore=db => db.pancakeFactory_PairCreated,
          ~getKey=({id}) => id,
        )
      },
      pancakePair_Swap: {
        makeStoreOperatorEntity(
          ~inMemoryStore,
          ~makeMockDb=makeWithInMemoryStore,
          ~getStore=db => db.pancakePair_Swap,
          ~getKey=({id}) => id,
        )
      },
      pancakePair_Sync: {
        makeStoreOperatorEntity(
          ~inMemoryStore,
          ~makeMockDb=makeWithInMemoryStore,
          ~getStore=db => db.pancakePair_Sync,
          ~getKey=({id}) => id,
        )
      },
      token: {
...
//ENTITY EXECUTION
  mockDb->executeRowsEntity(
    ~inMemoryStore,
    ~getInMemTable=self => {self.pair},
    ~getKey=entity => entity.id,
  )
  mockDb->executeRowsEntity(
    ~inMemoryStore,
    ~getInMemTable=self => {self.pancakeFactory},
    ~getKey=entity => entity.id,
  )
  mockDb->executeRowsEntity(
    ~inMemoryStore,
    ~getInMemTable=self => {self.pancakeFactory_PairCreated},
    ~getKey=entity => entity.id,
  )
  mockDb->executeRowsEntity(
    ~inMemoryStore,
    ~getInMemTable=self => {self.pancakePair_Swap},
    ~getKey=entity => entity.id,
  )
  mockDb->executeRowsEntity(
    ~inMemoryStore,
    ~getInMemTable=self => {self.pancakePair_Sync},
    ~getKey=entity => entity.id,
  )
  mockDb->executeRowsEntity(
    ~inMemoryStore,
    ~getInMemTable=self => {self.token},
    ~getKey=entity => entity.id,
  )
}
